...
///
/// Usage:
/// 1. Run this demo: `cargo run --example debugger_demo`
/// 2. In another terminal: `cargo run --bin hdds-debugger`
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("=== HDDS Web Debugger Demo ===\n");
    ...
    println!("[*] Starting Admin API on 127.0.0.1:4243");
    let discovery_fsm = participant1.discovery(); // Get FSM from first participant
    let mut admin = AdminApi::bind("127.0.0.1", 4243, discovery_fsm)?;
    ...
    println!("+---------------------------------------------------+");
    println!("|  Web Debugger Instructions                        |");
    println!("+---------------------------------------------------+");
    println!("|  1. Open a new terminal                           |");
    println!("|  2. Run: cargo run -p hdds-gateway                |");
