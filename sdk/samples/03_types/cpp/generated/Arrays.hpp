...
namespace hdds_samples {
    ...
    struct Arrays {
        std::array<int32_t, 10> numbers;  // was: sequence<long, 10> numbers
        std::array<std::string, 5> names;  // was: sequence<string, 5> names
        std::array<std::array<float, 3>, 3> transform;  // was: sequence<sequence<float, 3>, 3> transform

        Arrays() = default;
        Arrays(std::array<int32_t, 10> numbers, std::array<std::string, 5> names, std::array<std::array<float, 3>, 3> transform)
            : numbers(std::move(numbers)), names(std::move(names)), transform(std::move(transform)) {}
        ...
        [[nodiscard]] int encode_cdr2_le(std::uint8_t* dst, std::size_t len) const noexcept {
            ...
            offset = cdr2::align_offset(offset, 4);
            {
                std::uint32_t seq_len = static_cast<std::uint32_t>(this->transform.size());
                if (!cdr2::can_write(len, offset, 4)) return -1;
                std::memcpy(dst + offset, &seq_len, 4);
                offset += 4;
            }
            for (std::size_t i = 0; i < this->transform.size(); ++i) {
                offset = cdr2::align_offset(offset, 4);
                {
                    std::uint32_t seq_len = static_cast<std::uint32_t>(this->transform[i].size());
                    if (!cdr2::can_write(len, offset, 4)) return -1;
                    std::memcpy(dst + offset, &seq_len, 4);
                    offset += 4;
                }
                for (std::size_t j = 0; j < this->transform[i].size(); ++j) {
                    offset = cdr2::align_offset(offset, 4);
                    if (!cdr2::can_write(len, offset, 4)) return -1;
                    std::memcpy(dst + offset, &(this->transform[i][j]), 4);
                    offset += 4;
                }
            }
        ...
        [[nodiscard]] int decode_cdr2_le(const std::uint8_t* src, std::size_t len) noexcept {
            ...
            if (!cdr2::can_read(len, offset, 4)) return -1;
            {
                ...
                std::memcpy(&seq_len, src + offset, 4);
                offset += 4;
                if (seq_len > 5) return -1; // Exceeds bounded size
                for (std::size_t i = 0; i < seq_len && i < 5; ++i) {
                    offset = cdr2::align_offset(offset, 4);
                    if (!cdr2::can_read(len, offset, 4)) return -1;
                    {
            ...
            if (!cdr2::can_read(len, offset, 4)) return -1;
            {
                ...
                std::memcpy(&seq_len, src + offset, 4);
                offset += 4;
                if (seq_len > 3) return -1; // Exceeds bounded size
                for (std::size_t i = 0; i < seq_len && i < 3; ++i) {
                    offset = cdr2::align_offset(offset, 4);
                    if (!cdr2::can_read(len, offset, 4)) return -1;
                    {
                        std::uint32_t seq_len;
                        ...
                        offset += 4;
                        if (seq_len > 3) return -1; // Exceeds bounded size
                        for (std::size_t j = 0; j < seq_len && j < 3; ++j) {
                            offset = cdr2::align_offset(offset, 4);
                            if (!cdr2::can_read(len, offset, 4)) return -1;
                            std::memcpy(&(this->transform[i][j]), src + offset, 4);
                            offset += 4;
                        }
                    }
                }
...
// PubSubType for Arrays
// Generated by hddsgen

/// TypeSupport class for Arrays
/// Provides DDS-compatible serialization/deserialization interface
class ArraysPubSubType {
public:
    using type = Arrays;

    static constexpr const char* type_name() noexcept { return "Arrays"; }

    /// Serialize data to CDR2 little-endian format
    /// @param data Pointer to the Arrays to serialize
    ...
    static int serialize(const void* data, std::uint8_t* buffer, std::size_t buffer_size) noexcept {
        ...
        const auto* typed = static_cast<const Arrays*>(data);
    ...
    /// Create a new instance of Arrays
    /// @return Pointer to newly allocated Arrays, or nullptr on failure
    static void* create_data() noexcept {
        return new (std::nothrow) Arrays();
    }

    /// Delete an instance of Arrays
    /// @param data Pointer to Arrays to delete
    static void delete_data(void* data) noexcept {
        delete static_cast<Arrays*>(data);
    }
